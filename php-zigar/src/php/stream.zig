...
pub const Stream = opaque {
    ...
    pub fn wrapper(self: *const @This()) *Wrapper {
        ...
        return c.get_stream_wrapper(zstrm);
    }
    ...
    pub fn getContext(self: *const @This()) ?*Context {
        const zstrm = castFrom(@This(), self);
        const zctx = c.get_stream_context(zstrm);
        return castTo(Context, zctx);
    }
    ...
    pub fn setWrapper(self: *const @This(), w: *Wrapper) void {
        const zstrm = castFrom(@This(), self);
        return c.set_stream_wrapper(zstrm, castFrom(Wrapper, w));
    }

    pub fn toValue(self: *const @This()) Value {
        var result: Value = undefined;
        c.set_zval_stream(&result, castFrom(@This(), self));
        return result;
    }

    pub fn openFile(path: *const String, mode: [*c]const u8, context: ?*Context, options: c_int) !*Stream {
        const p = path.slice();
        const src = @src();
    ...
    pub fn close(self: *@This(), destroy: bool) void {
        ...
        const options = switch (destroy) {
            ...
            false => pd.PHP_STREAM_FREE_KEEP_RSRC | c.PHP_STREAM_FREE_CALL_DTOR | c.PHP_STREAM_FREE_RELEASE_STREAM,
        };
    ...
    pub fn readDirectory(strm: *Stream, ent: *DirectoryEntry) bool {
        const result = switch (@hasDecl(c, "_php_stream_readdir")) {
            false => pi.php_stream_readdir(strm, ent), // 8.6
            true => pi._php_stream_readdir(strm, ent),
        };
        return result != null;
    }

    pub fn write(self: *@This(), buf: [*]const u8, size: usize) !usize {
        const zstrm = castFrom(@This(), self);
        const result = switch (@hasDecl(pd, "_php_stream_write")) {
            false => pi.php_stream_write(zstrm, buf, size), // 8.6
            true => pi._php_stream_write(zstrm, buf, size),
        };
        if (result < 0) return error.Failure;
        return @intCast(result);
    }
    ...
    pub fn seek(self: *@This(), offset: i64, whence: u32) !void {
        const zstrm = castFrom(@This(), self);
        const ops = pd.get_stream_handlers(zstrm);
        const flags = pd.get_stream_flags(zstrm);
    ...
    pub fn statPath(path: *const String, context: ?*Context, _: LookupFlags, out: *Stat) !void {
        const p = path.slice();
        var stat_buf: pd.php_stream_statbuf = undefined;
        const zctx = if (context) |ctx| castTo(Context, ctx) else null;
    ...
    pub fn makeDirectory(path: *const String, mode: u32, context: ?*Context) !void {
        ...
        const result = switch (@hasDecl(pd, "_php_stream_mkdir")) {
            false => pi.php_stream_mkdir(p.ptr, @intCast(mode), 0, context), // 8.6
            true => pi._php_stream_mkdir(p.ptr, @intCast(mode), 0, context),
        };
        if (result == 0) return error.Failure;
    }
    ...
    pub fn removeDirectory(path: *const String, context: ?*Context) !void {
        ...
        const result = switch (@hasDecl(pd, "_php_stream_rmdir")) {
            false => pi.php_stream_rmdir(p.ptr, 0, context), // 8.6
            true => pi._php_stream_rmdir(p.ptr, 0, context),
        };
        if (result == 0) return error.Failure;
    }
    ...
    pub const Wrapper = struct {
        pub fn getOp(path: []const u8, comptime name: []const u8) !@Tuple(&.{ Wrapper, @FieldType(Wrapper.Ops, name) }) {
