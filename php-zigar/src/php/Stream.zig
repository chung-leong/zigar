...
pub const Context = @import("Stream/Context.zig");
pub const Wrapper = @import("Stream/Wrapper.zig");

pub fn isStdIo(self: *const @This()) bool {
    const ops = c.get_stream_handlers(@ptrCast(self));
...
pub fn open(path: *const String, mode: [*c]const u8, context: ?*Context, options: c_int) !*@This() {
    const p = path.slice();
    const src = @src();
...
pub fn openDirectory(path: *String, options: c_int, context: ?*Context) !*@This() {
    const p = path.slice();
    const src = @src();
    const pctx: ?*c.php_stream_context = if (context) |ctx| @ptrCast(ctx) else null;
...
pub fn openDescriptor(fd: c_int, mode: [*c]const u8) !*@This() {
    const src = @src();
