...
pub const Any = union(enum) {
    ...
    pub fn getEx(self: @This(), source: anytype, transform: ?Transform) !Value {
        @setEvalBranchQuota(2000000);
        const S = @TypeOf(source.*);
        return switch (self) {
            inline else => |acc| {
                ...
                if (@hasField(A, "getEx")) {
                    return switch (args) {
                        ...
                        .table => try acc.getEx(&source.table, transform),
                        .none => try acc.getEx(transform),
                        .object => try acc.getEx(ZigObject(S).fromStructure(source).object(), transform),
                    };
                } else {
                    var value = switch (args) {
                        .both => try acc.get(source.buffer, &source.table),
                        .buffer => try acc.get(source.buffer),
                        .table => try acc.get(&source.table),
                        .none => try acc.get(),
    ...
    pub fn getElementEx(self: @This(), source: anytype, index: usize, transform: ?Transform) !Value {
        @setEvalBranchQuota(2000000);
        const S = @TypeOf(source.*);
        return switch (self) {
            inline else => |acc| {
                ...
                if (@hasField(A, "getElementEx")) {
                    return switch (args) {
                        ...
                        .table => try acc.getElementEx(&source.table, index, transform),
                        .object => try acc.getElementEx(ZigObject(S).fromStructure(source).object(), index, transform),
                        .none => try acc.getElementEx(index, transform),
                    };
                } else {
                    var value = switch (args) {
                        .both => try acc.getElement(source.buffer, &source.table, index),
