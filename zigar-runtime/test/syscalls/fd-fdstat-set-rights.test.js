...
describe('Syscall: fd-fdstat-set-rights', function() {
  it('should remove fd_write from the rights of a descriptor', async function() {
    const env = new Env();
    ...
    const [ error ] = await captureError(async () => {
      result2 = await env.fdWrite(fd, iovsAddress, 1, writtenAddress, true);
    });
  })
  it('should fail when attempt to add new rights', async function() {
    const env = new Env();
    if (process.env.TARGET === 'wasm') {
      env.memory = new WebAssembly.Memory({ initial: 1 });
    } else {
