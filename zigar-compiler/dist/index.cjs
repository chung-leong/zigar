...
async function isOlderThan(targetPath, srcPaths) {
  try {
    const targetInfo = await promises.stat(targetPath);
    const checked = new Map();
    const check = async (path$1) => {
      if (!path$1) {
        return false;
      }
      /* c8 ignore next 3 */      
      ...
      if (info.isFile()) {
        ...
        const list = await promises.readdir(path$1);
        for (const name of list) {
          if (name.startsWith('.') || name === 'node_modules' || name === 'zig-cache') {
            continue;
          }
          if (await check(path.join(path$1, name))) {
            return true;
          }
        }
      }
  ...
  if (platform === 'linux') {
    ...
    if (isGNU === undefined) {
      ...
      if (process.versions?.electron || process.__nwjs) {
        ...
        try {
          childProcess.execFileSync('getconf', [ 'GNU_LIBC_VERSION' ], { stdio: 'pipe' });
          isGNU = true;
          /* c8 ignore next 3 */
        } catch (err) {
          isGNU = false;
  ...
  if (srcPath) {
    ...
    if (changed) {
      ...
      try {
        ...
        const now = new Date();
        await promises.utimes(outputPath, now, now);
      } finally {
        if (config.clean) {
...
function embed(path$1, dv, extra) {
