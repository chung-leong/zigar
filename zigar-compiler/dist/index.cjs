...
function createGlobalErrorSet() {
  globalErrorSet$2 = function() {};
  Object.setPrototypeOf(globalErrorSet$2.prototype, Error.prototype);
}
...
function flagMemberUsage(member, features) {
  const { type } = member;
  switch (type) {
    case MemberType$2.Bool:
      features.useBool = true;
      if (!isByteAligned$2(member)) {
        features.useExtendedBool = true;
      }
      break;
    case MemberType$2.Int:
      features.useInt = true;
      if(!isByteAligned$2(member) || !hasStandardIntSize(member)) {
        features.useExtendedInt = true;
      }
      break;
    case MemberType$2.Uint:
      features.useUint = true;
      if(!isByteAligned$2(member) || !hasStandardIntSize(member)) {
        features.useExtendedUint = true;
      }
      break;
    case MemberType$2.Float:
      features.useFloat = true;
      if (!isByteAligned$2(member) || !hasStandardFloatSize(member)) {
        features.useExtendedFloat = true;
      }
      break;
    case MemberType$2.EnumerationItem: {
...
function flagStructureUsage(structure, features) {
  ...
  const [ name ] = Object.entries(StructureType$2).find(a => a[1] === type);
...
function addStructureDefinitions(lines, definition) {
  ...
  const defaultStructure = {
    constructor: null,
    typedArray: null,
    type: StructureType$2.Primitive,
    name: undefined,
    byteSize: 0,
    align: 0,
    isConst: false,
...
function getExports(structures) {
  ...
  for (const member of root.static.members) {
    ...
    if (isReadOnly$2(member.type) && legal.test(member.name)) {
      try {
...
function addMethods(s, env) {
  const add = (target, { methods }, pushThis) => {
    ...
    defineProperties$2(target, descriptors);
  };
...
function addStaticMembers(structure, env) {
  ...
  const descriptors = {};
  for (const member of members) {
    descriptors[member.name] = getDescriptor$2(member, env);
  }
  defineProperties$2(constructor, {
    valueOf: { value: getValueOf$2 },
    toJSON: { value: convertToJSON$2 },
    ...
    [SLOTS$2]: { value: template[SLOTS$2] },
    [PROPS$2]: { value: members.map(m => m.name) },
    [NORMALIZER$2]: { value: normalizeStruct$2 },
  });
  if (type === StructureType$2.Enumeration) {
    const enums = constructor[ITEMS$2];
    for (const { name, slot } of members) {
      if (name !== undefined) {
        // place item in hash to facilitate lookup, 
        const item = constructor[SLOTS$2][slot];
        if (item instanceof constructor) {
          // attach name to item so tagged union code can quickly find it
          defineProperties$2(item, { [NAME$2]: { value: name } });  
          const index = item[Symbol.toPrimitive]();
          enums[index] = enums[name] = item;          
        }      
      } else {
        // non-exhaustive enum
        defineProperties$2(constructor, { [MORE$2]: { value: true } });
      }
    }
  } else if (type === StructureType$2.ErrorSet) {
    const allErrors = getGlobalErrorSet$2();
    const errors = constructor[ITEMS$2];
    for (const { name, slot } of members) {
      let error = constructor[SLOTS$2][slot];
      ...
      if (previous) {
        if (!(previous instanceof constructor)) {
          ...
          const otherErrors = Object.values(otherSet[SLOTS$2]);
          const errorIndices = Object.values(constructor[SLOTS$2]).map(e => Number(e));
        ...
        error = constructor[SLOTS$2][slot] = previous;       
      } else {
        // set error message (overriding prototype) and add to hash
        defineProperties$2(error, { message: { value: deanimalizeErrorName(name) } });
...
class Environment {
  ...
  */
  ...
  castView(structure, dv, writable) {
    ...
    const object = constructor.call(ENVIRONMENT$2, dv, { writable });
    if (hasPointer) {
  ...
  readSlot(target, slot) {
    const slots = target ? target[SLOTS$2] : this.slots;
    return slots?.[slot];
  }
  ...
  defineFactoryArgStruct() {
    ...
    const options = this.beginStructure({
      type: StructureType$2.Struct,
      name: 'Options',
      byteSize: 2,
      hasPointer: false,
    });
    this.attachMember(options, {
      type: MemberType$2.Bool,
      name: 'omitFunctions',
      bitOffset: 0,
      bitSize: 1,
      byteSize: 1,      
    ...
    const structure = this.beginStructure({
      type: StructureType$2.ArgStruct,
      name: 'factory',
      byteSize: 2,
      hasPointer: false,
    });
    this.attachMember(structure, {
      type: MemberType$2.Object,
      name: '0',
      bitOffset: 0,
      bitSize: 16,
      byteSize: 2,
  ...
  useStructures() {
    ...
    const objects = findAllObjects(this.structures, SLOTS$2);    
    for (const object of objects) {
      if (object[MEMORY$2] && this.inFixedMemory(object)) {
  ...
  createShadow(object) {
    ...
    const align = object.constructor[ALIGN$2] ?? dv[ALIGN$2];
    const shadow = Object.create(object.constructor.prototype);
    const shadowDV = shadow[MEMORY$2] = this.allocateShadowMemory(dv.byteLength, align);
    shadow[ATTRIBUTES] = {
  ...
  addShadow(shadow, object, align) {
    ...
    this.registerMemory(shadow[MEMORY$2], object[MEMORY$2], align);
    return shadow;
  }
  ...
  acquirePointerTargets(args) {
    ...
    const callback = function({ isActive, isMutable }) {
      ...
      const currentTarget = pointer[SLOTS$2][0];
      ...
      if (isActive(this)) {
        ...
        if (!currentTarget || isMutable(this)) {
          ...
          location = pointer[LOCATION_GETTER$2]();
          if (!isInvalidAddress(location.address)) {
            // get view of memory that pointer points to
            const len = (Target[SIZE$2] !== undefined) ? location.length * Target[SIZE$2] : undefined;
            const dv = env.findMemory(location.address, len);
            // create the target
            newTarget = Target.call(ENVIRONMENT$2, dv, { writable });
          } else {
            newTarget = null;
          }
        } else {
      ...
      currentTarget?.[POINTER_VISITOR$2]?.(callback, { vivificate: true, isMutable: () => writable });
      if (newTarget !== currentTarget) {
        newTarget?.[POINTER_VISITOR$2]?.(callback, { vivificate: true, isMutable: () => writable });
        pointer[SLOTS$2][0] = newTarget;
        if (env.inFixedMemory(pointer)) {
          pointer[FIXED_LOCATION$2] = location;
        }
      }
    };
    args[POINTER_VISITOR$2](callback, { vivificate: true });
  }

  /* COMPTIME-ONLY */
  acquireDefaultPointers(structure) {
    const { constructor, hasPointer, instance: { template } } = structure;
    if (hasPointer && template && template[MEMORY$2]) {
      // create a placeholder for retrieving default pointers
      const placeholder = Object.create(constructor.prototype);
      placeholder[MEMORY$2] = template[MEMORY$2];
      placeholder[SLOTS$2] = template[SLOTS$2];
...
class WebAssemblyEnvironment extends Environment {
  ...
  allocateHostMemory(len, align) {
    ...
    const constructor = { [ALIGN$2]: align };
    const copier = getMemoryCopier$2(len);
    const dv = this.allocateRelocMemory(len, align);
    const shadowDV = this.allocateShadowMemory(len, align);
    // create a shadow for the relocatable memory
    const object = { constructor, [MEMORY$2]: dv, [COPIER$2]: copier };
    const shadow = { constructor, [MEMORY$2]: shadowDV, [COPIER$2]: copier };
  ...
  obtainFixedView(address, len) {
    ...
    dv[MEMORY$2] = { memory, address, len };
    return dv;
  }
  ...
  inFixedMemory(object) {
    ...
    restoreMemory$2.call(object);
    return object[MEMORY$2].buffer === this.memory.buffer;
  }

  copyBytes(dst, address, len) {
    const { memory } = this;
    const src = new DataView(memory.buffer, address, len);
    const copy = getCopyFunction$2(len);
    copy(dst, src);
  }
  ...
  captureString(address, len) {
    ...
    return decodeText$2(ta);
  }

  getTargetAddress(target, cluster) {
    if (this.inFixedMemory(target)) {
      return this.getViewAddress(target[MEMORY$2]);
    }
    if (target[MEMORY$2].byteLength === 0) {
...
async function transpile(path, options) {
  ...
  const srcPath = path.endsWith('.zig') ? path : findSourceFile$1(path, { 
