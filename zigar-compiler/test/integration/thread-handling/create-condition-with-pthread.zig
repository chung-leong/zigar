...
extern fn pthread_condattr_setclock(*pthread_condattr_t, c_int) c_int;
...
pub fn spawn() !void {
    ...
    if (@hasDecl(c, "pthread_condattr_setclock")) {
        if (pthread_condattr_setclock(&attrs, c.CLOCK_REALTIME) != 0) return error.CannotSetConditionAttribute;
    }
