...
export function addTests(importModule, options) {
  ...
  describe('Opaque', function() {
    ...
    it('should handle opaque pointer in array', async function() {
      ...
      expect(module.array[2]['*']).to.be.instanceOf(AnyOpaque);
      expect(module.array[3]['*']).to.be.instanceOf(AnyOpaque);
    ...
    it('should handle opaque pointer in struct', async function() {
      this.timeout(120000);
      const { default: module, StructA, AnyOpaque, print } = await importTest('in-struct');
      expect(module.struct_a.ptr1['*']).to.be.instanceOf(AnyOpaque);
      expect(module.struct_a.ptr2['*']).to.be.instanceOf(AnyOpaque);
      const b = new StructA({});
      expect(b.ptr1['*']).to.be.instanceOf(AnyOpaque);
      expect(b.ptr2['*']).to.be.instanceOf(AnyOpaque);
      const [ before ] = await capture(() => print());
    ...
    it('should handle opaque pointer in tagged union', async function() {
      this.timeout(120000);
      const { default: module, TagType, UnionA, AnyOpaque, print } = await importTest('in-tagged-union');
      expect(module.union_a.ptr['*']).to.be.instanceOf(AnyOpaque);
      ...
      const c = new UnionA({ number: 123 });
      expect(b.ptr['*']).to.be.instanceOf(AnyOpaque);
      expect(c.number).to.equal(123);
    ...
    it('should handle opaque pointer in optional', async function() {
      this.timeout(120000);
      const { default: module, AnyOpaque, print } = await importTest('in-optional');
      expect(module.optional['*']).to.be.instanceOf(AnyOpaque);
      const [ before ] = await capture(() => print());
    ...
    it('should handle opaque pointer in error union', async function() {
      this.timeout(120000);
      const { default: module, Error, AnyOpaque, print } = await importTest('in-error-union');
      expect(module.error_union['*']).to.be.instanceOf(AnyOpaque);
      const [ before ] = await capture(() => print());
      expect(before).to.equal('1234');
