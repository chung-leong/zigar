...
const HIDE_PREVIOUS = Symbol('hidePrevious');
...
function throwMissingUnionInitializer(structure, arg) {
...
function throwArraySizeMismatch(structure, count, arg) {
  const { name } = structure;
...
function getDataView(structure, arg, TypedArray = null) {
  let dv = null;
...
function finalizeArray(s) {
  ...
  const constructor = s.constructor = function(arg) {
    ...
    if (objectMember) {
      createChildObjects$1.call(self, objectMember, 0, this, dv);
    }
    if (creating) {
...
function createChildObjects$1(member, startOffset, recv, dv) {
...
function addPointerAccessors(s) {
  const {
    constructor,
  ...
  for (const [ target, members ] of list) {
    ...
    for (const member of members) {
      const accessors = getPointerAccessors(member);
      if (accessors) {
...
function getPointerAccessors(member, options) {
...
function finalizeSlice(s) {
  ...
  const initializer = s.initializer = function(arg) {
    ...
    if (objectMember) {
      if (size > currentSize) {
        createChildObjects$1.call(this, objectMember, currentSize, this, dv);
...
async function linkWASMBinary(binaryPromise, params = {}) {
  const {
    resolve,
    reject,
    promise,
...
export { finalizeStructures, linkWASMBinary, runWASMBinary, serializeDefinitions, useArgStruct, useArray, useBareUnion, useBool, useBoolEx, useEnumeration, useEnumerationItem, useEnumerationItemEx, useErrorSet, useErrorUnion, useExternUnion, useFloat, useFloatEx, useInt, useIntEx, useObject, useOpaque, useOptional, usePointer, usePrimitive, useSlice, useStruct, useTaggedUnion, useType, useVoid };
